...
vec3 StarLayer(vec2 uv) {
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      // Constrain to white + blue/silver palette — no yellow or purple
      float starVariance = Hash21(si + 5.0);
      float hue = uHueShift / 360.0 + (starVariance - 0.5) * 0.055;
      float sat = pow(starVariance, 2.5) * uSaturation;
...
export default function Galaxy({
  ...
  transparent = true,
  eventEl = null, // optional ref to a parent element to listen on instead of canvas container
  ...rest
}) {
